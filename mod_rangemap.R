...
range_map_lf <- function(dataframe, type){
  ...
  Counties.t <- sf::st_transform(Counties, crs = 4326)
  Species_observations.t <- sf::st_transform(dataframe, crs = 4326)
...
observation_map_lf <- function(dataframe){
  ...
  Species_observations <- sf::st_transform(dataframe, crs = 4326)
  ...
  Map <- leaflet()%>%
    addPolylines(data = States,
                 color = "Black",
                 weight = 2)%>%
    addCircles(data = Species_observations,
               fillColor = "#A50F15",
               fillOpacity = 0.7,
               color = "#A50F15",
               radius = 800,
