...
regions <- phys %>%
  select(US_L3NAME)%>%
  filter(US_L3NAME%in%c("Southeastern Plains", "Southern Coastal Plain", "Middle Atlantic Coastal Plain",
...
construct_pheno <- function(regionx){
  ...
  pheno_georgia <- if(regionx == "All"){
    GeorgiaObs%>%
      ...
      dplyr::mutate(obs.sp=sum(obs))%>%
      dplyr::mutate(obs.scale = scale(obs))%>%
      dplyr::mutate(prop = (obs/obs.sp))%>%
      dplyr::mutate(prop.scaled = scale(prop))%>%
      dplyr::mutate(weeklyavg = rollmean(x = obs, k = 7, fill = NA, align = c("center")))%>%
      dplyr::mutate(freq.7day.avg = rollmean(x = prop, k = 7, fill = NA, align = c("center")))%>%
...
pheno_plot <- function(regionx){
  ...
  plot <- ggplot(pheno_georgia)+
    ...
    # geom_smooth(aes(`Day of Year`, `Weekly Average Observations`),
    #             se = T, alpha = 0.2, color = "#A50F15")+
...
phys_map <- function(){
  ...
  Map <- ggplot()+
    ...
    scale_fill_brewer(type = "qual", palette = "Paired", 
                      direction = 1)+
    theme_void()+
    labs(fill = "Physiographic Region")+
